...
def Algorithm_TSP(start_node, n_node):
	...
	while i<n_node:
		j=i+1
		while j<=n_node:
			edges.append(Edge(i,j,math.sqrt((dict_index_coord[start_node][0]-dict_index_coord[i][0])**2+(dict_index_coord[start_node][1]-dict_index_coord[i][1])**2)+math.sqrt((dict_index_coord[start_node][0]-dict_index_coord[j][0])**2+(dict_index_coord[start_node][1]-dict_index_coord[j][1])**2)-math.sqrt((dict_index_coord[i][0]-dict_index_coord[j][0])**2+(dict_index_coord[i][1]-dict_index_coord[j][1])**2)))
			j=j+1		
		i=i+1
	edges=sorted(edges)
...
class Graph(QMainWindow):
	...
	def findClosestCoordinate(self, min_x, min_y, x, y):
		...
		if self.event == "Generate_Graph":
			...
			Algorithm_TSP(start_node, self.nodes)
